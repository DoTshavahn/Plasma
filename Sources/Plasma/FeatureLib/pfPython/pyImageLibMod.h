...
protected:
    ...
    pyImageLibMod() // for python glue only, do NOT call
    {
        fModifierKey = nullptr;
        fModifier = nullptr;
    }
...
public:
    ...
    // for C++ access
    plKey GetKey() { return fModifier ? fModifier->GetKey() : fModifierKey; }

    // for python access
    plBitmap* GetImage(const ST::string& name);
    std::vector<ST::string> GetImageNames();
};
